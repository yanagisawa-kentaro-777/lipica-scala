...
class TransportMessagesTest extends Specification {
	...
	"test PingMessage" should {
		"be right" in {
			...
			val message3: PingMessage = TransportMessage.decode(message2.packet).right.get
	...
	"test PongMessage" should {
		"be right" in {
			...
			val message1: PongMessage = TransportMessage.decode(wire).right.get

			val key = ECKey.fromPrivate(BigInteger.TEN)
			val message2 = PongMessage.create(message1.token, key)
			val message3: PongMessage = TransportMessage.decode(message2.packet).right.get
	...
	"test FindNodeMessage" should {
		"be right" in {
			...
			val message1: FindNodeMessage = TransportMessage.decode(wire).right.get

			val key = ECKey.fromPrivate(BigInteger.TEN)
			val message2 = FindNodeMessage.create(message1.target, key)
			val message3: FindNodeMessage = TransportMessage.decode(message2.packet).right.get
	...
	"test NeighborsMessage" should {
		"be right" in {
			...
			val message1: NeighborsMessage = TransportMessage.decode(wire).right.get

			val key = ECKey.fromPrivate(BigInteger.TEN)
			val message2 = NeighborsMessage.create(message1.nodes, key)
			val message3: NeighborsMessage = TransportMessage.decode(message2.packet).right.get
