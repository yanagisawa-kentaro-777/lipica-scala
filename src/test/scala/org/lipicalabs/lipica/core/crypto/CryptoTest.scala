...
class CryptoTest extends Specification {
	...
	"test (1)" should {
		"be right" in {
			...
			val addr = ECKeyPair.fromPrivateKey(privateKey).toAddress
	...
	"test (2)" should {
		"be right" in {
			val key = ECKeyPair.fromPrivateKey(Hex.decodeHex("a4627abc2a3c25315bff732cb22bc128f203912dd2a840f31e66efb27a47d2b1".toCharArray))

			val address = key.toAddress.toHexString
			val publicKey = Hex.encodeHexString(key.publicKeyPoint.getXCoord.getEncoded) + Hex.encodeHexString(key.publicKeyPoint.getYCoord.getEncoded)
