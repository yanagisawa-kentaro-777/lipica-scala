...
trait BlockStore extends Closeable {
	 ...
	 */
	def getBlockHashByNumber(blockNumber: Long, branchBlockHash: DigestValue): Option[DigestValue]

	/**
	 * ブロック番号で、ブロックハッシュを引いて返します。
	 * （メインチェーンのみ）
	 */
	def getBlockHashByNumber(blockNumber: Long): Option[DigestValue]
	 ...
	 */
	def getHashesEndingWith(hash: DigestValue, count: Long): Seq[DigestValue]
	 ...
	 */
	def getTotalDifficultyForHash(hash: DigestValue): BigInt
